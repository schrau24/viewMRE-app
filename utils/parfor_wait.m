classdef parfor_wait < handle
   ...
   methods
       function Obj = parfor_wait(TotalMessage, varargin)
           ...
           if Obj.Waitbar
               Obj.WaitbarHandle = waitbar(0, ['Registering frames: ' num2str(0), '%'], 'Resize', true);
           end
           switch Obj.FileName
               case 'screen'
               otherwise
...
%            waitbar(Obj.NumMessage/Obj.TotalMessage, Obj.WaitbarHandle, [num2str(Obj.NumMessage/Obj.TotalMessage*100, '%.2f'), '%; ', num2str(UsedTime_now, '%.2f'), 's used and ', num2str(EstimatedTimeNeeded, '%.2f'), 's needed.']);
           waitbar(Obj.NumMessage/Obj.TotalMessage, Obj.WaitbarHandle, ['Registering frames: ' num2str(Obj.NumMessage/Obj.TotalMessage*100, '%.2f'), '%']);
