...
% Eric, tomoelastography updates, May 2021
scanDate = strrep(header.datetime(1:10),'.',''); scanDate = str2double([scanDate(5:8) scanDate(3:4) scanDate(1:2)]);
if scanDate > 202105
    % for R571 (after May 2021), shift header.tbl(:,42) to echo header.tbl(:,2), and update
    % header.sizes
    header.tbl(:,2) = header.tbl(:,42);
    header.sizes(4) = numel(unique(header.tbl(:,2)));
    header.nechoes  = header.sizes(4);
end
A = header.tbl;
